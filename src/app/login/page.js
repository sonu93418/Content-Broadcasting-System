    ...
    <div className="min-h-screen flex items-center justify-center bg-[#030712] px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-in">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-text-secondary">Content Broadcasting System</p>
        </div>

        {/* Role selector */}
        {!selectedRole && (
          <div className="space-y-4 animate-fade-in">
            <h2 className="text-lg font-semibold text-text-primary text-center mb-2">Choose your role</h2>

            <button onClick={() => handleRoleSelect('teacher')} className="w-full glass-card p-5 flex items-center gap-4 hover:border-primary/40 transition-all duration-300 hover:-translate-y-0.5 group text-left">
              ...
              <div>
                <h3 className="text-base font-semibold text-text-primary">Sign in as Teacher</h3>
                <p className="text-xs text-text-secondary mt-0.5">Upload content, manage schedules, track approvals</p>
              </div>
            </button>

            <button onClick={() => handleRoleSelect('principal')} className="w-full glass-card p-5 flex items-center gap-4 hover:border-success/40 transition-all duration-300 hover:-translate-y-0.5 group text-left">
              ...
              <div>
                <h3 className="text-base font-semibold text-text-primary">Sign in as Principal</h3>
                <p className="text-xs text-text-secondary mt-0.5">Review content, approve/reject, manage broadcasts</p>
              </div>
            </button>
          </div>
        )}
        ...
        {selectedRole && (
          <div className="glass-card p-8 animate-scale-in">
            <div className="flex items-center gap-3 mb-6">
              <button onClick={() => { setSelectedRole(null); clearError(); reset({ email: '', password: '' }); }} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-surface-lighter transition-colors">
                <HiOutlineArrowLeft className="w-4 h-4 text-text-secondary" />
              </button>
              <div>
                <h2 className="text-lg font-semibold text-text-primary capitalize">{selectedRole} Login</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  <HiOutlineEnvelope className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-text-muted" />
            ...
            <div className="mt-5 pt-5 border-t border-border">
              ...
              <div className="bg-surface rounded-xl p-3 text-center">
                <p className="text-xs text-text-secondary">{DEMO_CREDS[selectedRole]?.email} / {DEMO_CREDS[selectedRole]?.password}</p>
              </div>
            </div>
          </div>
        )}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center bg-[#030712]"><div className="w-12 h-12 border-3 border-primary/30 border-t-primary rounded-full animate-spin" /></div>}>
      <LoginForm />
    </Suspense>
  );
}
